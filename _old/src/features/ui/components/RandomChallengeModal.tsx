...
const DIFFICULTY_LABELS = {
  ...
  ADVANCE: { en: 'Advance', zh: '高级' },
  EXPERT: { en: 'Expert', zh: '专家' },
  GRANDMASTER: { en: 'Grandmaster', zh: '大师' },
  ASTRAL: { en: 'Astral', zh: '星辉' },
  CELESTIAL: { en: 'Celestial', zh: '天极' },
};
...
export const RandomChallengeModal: React.FC = () => {
  ...
  const getDifficultyLabel = (val: number) => {
    if (val < 1) return DIFFICULTY_LABELS.BEGINNER;
    if (val < 2) return DIFFICULTY_LABELS.INTERMEDIATE;
    if (val < 3) return DIFFICULTY_LABELS.ADVANCE;
    if (val < 4) return DIFFICULTY_LABELS.EXPERT;
    if (val < 5) return DIFFICULTY_LABELS.GRANDMASTER;
    if (val < 6) return DIFFICULTY_LABELS.ASTRAL;
    return DIFFICULTY_LABELS.CELESTIAL;
  };
  ...
  const handleStart = async () => {
    ...
    // 使用本地拓扑变换算法生成
    const result = await generateFunctionByDifficulty({ targetDifficulty: difficulty, withParams });
    ...
    const encodedLevel = btoa(encodeURIComponent(JSON.stringify({ t: result.target, p: result.params, d: difficulty, wp: withParams })));
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex justify-center items-center pointer-events-auto">
      ...
      {/* Modal */}
      <div className="relative w-full h-full md:h-auto md:max-w-[450px] bg-background text-foreground md:rounded-[16px] shadow-modal overflow-hidden border-none md:border md:border-border flex flex-col animate-zoom-in md:m-4">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-[32px] bg-background custom-scrollbar flex flex-col gap-8">
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="w-full bg-card border border-border p-[20px] rounded-[16px] shadow-sm flex flex-col items-center gap-4 hover:border-primary/50 transition-colors">
          ...
          <div className="text-center mt-2">
            <button 
              ...
              className={`w-full ${isGenerating ? 'bg-primary/50 cursor-not-allowed' : 'bg-primary hover:brightness-110 shadow-btn hover:shadow-btn-hover active:scale-[0.98]'} text-white py-[14px] text-[1.15rem] font-bold rounded-[12px] transition-all flex items-center justify-center gap-2`}
            >
              {isGenerating ? (
                <>
