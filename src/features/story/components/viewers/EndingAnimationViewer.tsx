...
export const EndingAnimationViewer: React.FC<{ content: string, onClose: () => void }> = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    const timer1 = setTimeout(() => setStage(1), 500);  // Background fades in
    const timer2 = setTimeout(() => setStage(2), 2500); // Line 1
    const timer3 = setTimeout(() => setStage(3), 6000); // Line 2
    const timer4 = setTimeout(() => setStage(4), 9500); // Line 3
    const timer5 = setTimeout(() => setStage(5), 12500); // Lines 1-3 fade out
    const timer6 = setTimeout(() => setStage(6), 14500); // Core line
    const timer7 = setTimeout(() => setStage(7), 18000); // Core line out
    const timer8 = setTimeout(() => setStage(8), 20000); // Sub line in
    const timer9 = setTimeout(() => setStage(9), 23500); // Sub line out
    const timer10 = setTimeout(() => setStage(10), 25500); // Start Credits
    // timer11 and timer12 are removed because credits end is now dynamic based on animation end
  ...
  const content = (
    ...
    >
      ...
      {/* Credits Roll */}
      {stage >= 10 && (
        <div className="absolute inset-0 flex justify-center w-full z-20 pointer-events-none">
          <style>
            {`
          ...
          >
            <div className="flex flex-col items-center gap-16 md:gap-24 w-full px-8 py-12">
              ...
              <div className="mt-32 mb-16 pb-[50vh]">
