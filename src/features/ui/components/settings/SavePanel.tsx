...
const SaveDataSchema = z.object({
  state: z.object({
    ...
    seenChapters: z.array(z.string()).optional(),
...
const SettingsOption = ({ icon: Icon, label, onClick, isDanger, isFile, rightContent }: { icon: React.ElementType, label: string, onClick?: (e: React.MouseEvent<HTMLButtonElement> | React.ChangeEvent<HTMLInputElement>) => void, isDanger?: boolean, isFile?: boolean, rightContent?: React.ReactNode }) => {
  const baseClasses = `group relative overflow-hidden flex items-center justify-between px-[20px] py-[16px] text-[1.05rem] font-medium bg-card text-foreground border border-border rounded-[10px] transition-all duration-200 ease-[cubic-bezier(0.4,0,0.2,1)] hover:-translate-y-[2px] w-full cursor-pointer`;
  
  const hoverClasses = isDanger 
    ? `hover:shadow-lg hover:border-destructive hover:bg-destructive/10 hover:text-destructive`
    : `hover:shadow-lg hover:border-primary hover:bg-primary/10 hover:text-foreground`;
    
  const barColor = isDanger ? 'bg-destructive' : 'bg-primary';
  const textColor = isDanger ? 'text-destructive' : '';
...
export const SavePanel: React.FC = () => {
  ...
  const getSlotKey = (slot: string) => `${GAME_CONSTANTS.STORAGE_KEYS.SLOT_PREFIX}${slot}`;

  const hasSaveData = (slot: string) => {
    if (slot === currentSlot) {
      const state = useGameStore.getState();
      return (state.completedLevels?.length ?? 0) > 0;
    }
    ...
    try {
      ...
      return (parsedData.state?.completedLevels?.length ?? 0) > 0;
    } catch {
      return false;
    }
  };
  ...
  const handleSlotSelect = (slot: string) => {
    ...
    if (newData) {
      try {
        const rawData = JSON.parse(newData);
        const parsed = SaveDataSchema.parse(rawData);
        if (parsed.state) {
        ...
        console.error(SYSTEM_LOGS.ERROR_LOAD_SLOT, e);
      }
    } else {
    ...
    setCurrentSlot(slot);
    localStorage.setItem(GAME_CONSTANTS.STORAGE_KEYS.CURRENT_SLOT, slot);
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement> | React.MouseEvent<HTMLButtonElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const data = ExportDataSchema.parse(rawData);
  ...
  const handleConfirmClear = (e?: React.MouseEvent) => {
    ...
    // 从触发位置发射红色的删除特效
    import('../../../../utils/confettiHelper').then(({ fireClearConfetti }) => {
      // confetti需要的是基于视口的比例 0-1, x和y已经是计算好的比例了
      fireClearConfetti(x, y);
    });
            ...
            return (
              <button
                ...
                className={`flex items-center justify-between w-full px-4 py-3 border transition-all rounded-xl transform hover:-translate-y-0.5 ${
                  isCurrent 
                    ? 'border-primary bg-[rgba(var(--primary-color-rgb),0.1)] text-primary' 
                    : 'border-border bg-card text-foreground hover:border-primary hover:bg-card-hover'
                }`}
              >
