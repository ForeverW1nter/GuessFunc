...
故事模式的数据全部存储在 `src/assets/data/` 目录下的 JSON 文件中（例如 `story.json`, `test_route.json` 等）。你可以将不同的故事线拆分到多个 JSON 文件中，系统会自动将它们合并。
它的层级结构如下：
...
1. 在 `src/assets/data/` 目录下新建一个 JSON 文件，或者打开已有的 JSON 文件。
...
## 4. 使用可视化编辑器
为了方便非技术人员创作，我们提供了一个内置的**「关卡/剧情编辑器」**工具。
在开发环境中，你可以通过浏览器访问 `/#/tools/story-editor`（或在相关菜单中找到入口），在网页上直接配置参数、编写 Markdown 剧情，并一键导出合规的 JSON 文件！
